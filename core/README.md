...
| Path | What it is |
|---|---|
| [`conventions.md`](conventions.md) | The format contract, claim protocol, command hygiene, and git/clarify modes — the rules every agent and script obeys |
| [`AGENT-CONTEXT.md`](AGENT-CONTEXT.md) | The one page of `conventions.md` that must bind *before* anything points at it — imported into the project's instruction file so it is in context from the first message |
| `templates/` | The starting shape of each living document (vision, roadmap, progress, queue, item) |
| `scripts/aide.py` | The stdlib CLI — `python .aide/scripts/aide.py {check,scope,progress,queue,claim,merge,env,sync,gc,status}` |

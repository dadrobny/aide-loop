...
**`gc` asks git, not the document.** On the ✅ ground `gc` deletes a branch only
when `git merge-tree --write-tree` says merging it into the base would change
nothing. A ✅ item whose branch still carries unlanded content is **skipped**
...
- **Why `gc` asks git.** A ✅ is a claim made by a document that agents and
  ...
  delete — unrecoverable on a plain git host. The merge-tree question is the
  content question, which (unlike `git branch --merged`) stays correct across
  a squash merge, and which also strengthens `--merged`. Refusing on old git
  keeps old git always *more* conservative, never less.

...
It is distributed as this repository and nothing else: a consumer clones it or
unpacks a release archive, runs `install.py` from that tree, and reports back
to this tracker. There is no package, registry or hosted service in between,
so what a consumer runs is what it can read, and a fix is a version a consumer
pulls.

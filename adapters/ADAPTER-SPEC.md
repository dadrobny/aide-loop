...
themselves live in `conventions.md` §3 and are runtime-general — an adapter's
guard enforces that section's bullets, carve-out included, and this document
does not summarise them; only the *enforcement mechanism* and the "permission
allow-list" framing are adapter-local. A runtime with no permission

"""A delivered rule still says what its section says — asserted, not reviewed.

The gap this closes (issue #81). ADAPTER-SPEC §7 splits the contract in two:
`core/conventions/` holds the rule, `.claude/rules/` delivers it, and the
...
precedent from #84: `test_rules.py` asserts the frontmatter keys are exactly
`["paths"]`, a comment carries no runtime meaning, it works on an unscoped rule
that has no frontmatter at all, and it survives #85's possible move of this
content into skills. One block per section file, so a rule delivering four
sections declares four blocks. A `- ` opens a pin and any line under it that
does not continues it, so a pin may wrap.
...
def _id(case) -> str:
    ...
    return f"{rule.name}::{section.rsplit('/', 1)[-1]}::{pin[:48]}"


# --------------------------------------------------------------------------- #
# fail closed — the corpus is recognisable before anything is asserted about it
# --------------------------------------------------------------------------- #
def test_there_are_rules_and_they_declare_pins():
    ...
    assert _PINNED, "no rule declares a `<!-- pins: … -->` block"


@pytest.mark.parametrize("rule", _RULE_FILES, ids=lambda p: p.name)
def test_every_delivered_rule_pins_at_least_one_statement(rule: Path):
    """No rule ships unguarded, including one added tomorrow.

    A delivered rule that pins nothing is not cheap — it is a restatement with
    no counterpart named, which is the exact shape #79 shipped twice. If a rule
...
def test_a_pinned_statement_still_appears_in_the_section_it_names(case):
    ...
    assert section.is_file(), (
        f"{rule.name} pins {declared}, which is not a file — the section moved "
        f"or was renamed")
    assert _normalise(pin) in _normalise(_read(section)), (
        f"{rule.name}: pinned statement is no longer in {declared}\n"
        f"  pinned: {pin}\n"
...
def test_a_pinned_statement_still_appears_in_the_rule_that_declares_it(case):
    ...
    The rule body is searched with every HTML comment stripped, so a pin cannot
    satisfy itself. Deleting or rewording the delivered sentence while leaving
    the pin behind fails here — the pin list stops being a description of the
    rule and becomes part of it.
    """
    rule, _, pin = case
    ...
    assert _normalise(pin) in _normalise(body), (
        f"{rule.name}: declares a pin its own body no longer states\n"
        f"  pinned: {pin}\n"

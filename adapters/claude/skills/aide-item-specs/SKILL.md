...
<!-- reach: spec-author
     ...
     specs whose `skills:` frontmatter names `aide-item-specs` — the one role
     that writes an item spec. Four sections, because the spec-author writes
...
<!-- pins: .aide/conventions/1-format-contract/authorised-paths.md
     - An item spec declares the files it may change, and `aide scope` proves
       the declaration by the diff
     - The section is **expected but not required**
     - One path per bullet is a contract, not a style note
     - the **first backtick span before its reason** and nothing else
     - **May change** — every path this item is authorised to modify
     - Prefer the narrowest form that covers the work
     - **Asserts against** — files or derived artifacts this item's tests read
...
<!-- pins: .aide/conventions/1-format-contract/environment-gated-capabilities.md
     - its tests skip cleanly (never fail, never silently pass as if
       exercised) when the dependency is absent
     - That graceful-fallback bar is enough for a stage to reach ✅ under the
       rollup rule
